...
## [0.5.0] 2026-04-17 — 15-Skill Complete Pipeline + Knowledge Management

### Added

**15-Skill Architecture**
1. `df-preprocess` — Data ingestion, validation, quality gates, incremental processing
2. `df-eda` — Exploratory analysis (per-column + global + domain insights)
3. `df-feature-architect` — Feature engineering, encoding, selection, store design
4. `df-modeling` — Model training, evaluation, interpretation, fairness
5. `df-deploy` — Model serving (batch/online/streaming), optimization, monitoring
6. `df-experiment` — Experiment tracking, drift detection, performance monitoring
7. `df-pipeline` — End-to-end orchestration, stage gates, checkpoints
8. `df-analysis` — Workflow: preprocess → EDA → report
9. `df-knowledge` — Live knowledge base (read-only)
10. `df-learn` — Knowledge base updates, learning extraction
11. `df-llm-orchestrator` — LLM selection, caching, evaluation, guardrails, routing
12. `df-mlops-pipeline` — CI/CD, orchestration, event-driven workflows, streaming pipelines
13. `df-distributed-training` — Multi-GPU training, parallel processing, hyperparameter search
14. `df-rag-orchestrator` — RAG pipeline, chunking, embedding, retrieval, generation
15. `df-report` — HTML/PDF report generation

**Router Aliasing**
- Both `/dataforge` and `/df` command prefixes route to same skills (user choice for invocation)

### Changes

**Documentation Updates**
- `skills/df-*/README.md` — 15 per-skill READMEs (user-focused quick start guides)
- `CLAUDE.md` — Updated architecture section with 15-skill diagram and routing table
- `docs/ARCHITECTURE.md` — Complete system design with layer diagram
- `docs/COMMANDS.md` — Full command reference for all 15 skills

**Router Enhancements**
- `skills/df/SKILL.md` — Router with skill delegation logic (supports both `/dataforge` and `/df` prefixes)
- `skills/dataforge/SKILL.md` — Alias router for backward compatibility
