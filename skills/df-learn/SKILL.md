...
description: >
  ...
  "refresh knowledge", "/df-learn", "update KB", "learn libraries".
user-invokable: true
...
|---------|-------------|
| `/df-learn all` | Refresh every stale area (libraries, domains, roles) |
| `/df-learn library [--area sklearn]` | Refresh one library (or all libraries if no --area) |
| `/df-learn domain [--area finance]` | Refresh one domain (or all domains) |
| `/df-learn role [--area mlops]` | Refresh one role (or all roles) |
...
| No sources.json or empty | Print "no sources configured, run /df-setup" and stop |
