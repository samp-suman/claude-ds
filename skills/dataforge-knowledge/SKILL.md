...
description: >
  ...
  "show knowledge", "/dataforge-knowledge", "KB status", "what does DataForge
  know about".
user-invokable: true
...
> use `/dataforge-learn` for that.

## Commands

| Command | What it does |
|---------|-------------|
| `/dataforge-knowledge show <area>` | Print the full live markdown for one area (e.g. `sklearn`, `finance`) |
| `/dataforge-knowledge search <query>` | Search entries across every live file by title/summary/body |
| `/dataforge-knowledge status` | Per-area freshness report: source, seeded_at, last_refreshed_at, stale? |
| `/dataforge-knowledge diff` | Show which files are seed-sourced vs researcher-sourced, with entry counts |
...
- If the user asks to refresh, suggest `/dataforge-learn` and stop.
