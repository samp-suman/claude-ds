...
| `/df-modeling visualize <project-dir>` | Generate evaluation plots (confusion matrix, ROC, etc.) |
...
- If missing: tell the user to run preprocessing first:
  `/df-preprocess features <dataset> <target>`
...
When running standalone (`/df-modeling train`), run the checkpoint here:
...
| Scenario | Action |
|----------|--------|
| Processed data missing | Tell user to run `/df-preprocess features` first |
